...
{
	if(argc != 5) {
		printf("Usage: ./tasks <n> <p> <bandwidth> <number of threads>\n");
	}
	...
	#pragma omp parallel default(none) shared(tmpHolder, data, still_shifting, max_min_dist, iteration_number, bw, n, p, shift_points) 
	{
		#pragma omp single 
	    {
	    double *tmpPointer;
